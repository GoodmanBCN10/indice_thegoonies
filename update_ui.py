...
content = re.sub(
    r'\.hero-unit \{.*?\}\s*\.avatar-container \{.*?\}\s*\.avatar-container::after \{.*?\}\s*@keyframes pulse-border \{.*?\}\s*\.avatar \{.*?\}\s*\.brand-titles \{.*?\}\s*\.brand-main \{.*?\}\s*\.brand-sub \{.*?\}',
    r'''.hero-unit { display: flex; align-items: center; gap: 25px; }
...
content = re.sub(
    r'\.stats-badge \{.*?\}.*?\.stats-count \{.*?\}\s*\.stats-label \{.*?\}',
    r'''.stats-badge {
...
# Mobile changes
content = re.sub(
    r'\.avatar-container \{ width: 80px; height: 80px; \}\s*\.brand-main \{ font-size: 2\.2rem; \}\s*\.brand-sub \{ font-size: 0\.8rem; letter-spacing: 4px; \}\s*\.stats-badge \{ padding: 10px 20px; width: 100%; \}',
    r'''.avatar-container { width: 70px; height: 70px; }
...
new_setCat = '''function setCategory(cat) {
            currentCategory = cat;
...
new_render = '''function renderInitial() {
            const term = document.getElementById('search').value.toLowerCase();
            const sortBy = document.getElementById('sort').value;
            if (term) {
                backBtn.style.display = 'block';
...
with open(target, 'w', encoding='utf-8') as f:
    f.write(content)
print('Updated UI script successfully!')
